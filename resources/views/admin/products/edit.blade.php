...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-1 fw-bold">{{ __('cms.products.title_edit') }}</h4>
    ...
    <div class="row">
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-body p-4">
                    <h6 class="fw-bold mb-4 pb-2 border-bottom">
                        <i class="bi bi-box-seam text-primary me-2"></i>{{ __('cms.products.section_product_info') }}
                    </h6>

                    <div class="mb-3">
                        <label class="form-label fw-semibold">{{ __('cms.products.product_name') }}</label>
                        <input type="text"
                            name="name"
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold">{{ __('cms.products.description') }}</label>
                        <textarea
                            name="description"
                    ...
                    <div class="mb-0">
                        <label class="form-label fw-semibold">{{ __('cms.products.short_description') }}</label>
                        <textarea
                            name="short_description"
                            class="form-control bg-light border-0"
                            rows="3"
                            placeholder="{{ __('cms.products.short_description_placeholder') }}">{{ old('short_description', $product->short_description) }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body p-4">
                    <div class="d-flex align-items-center justify-content-between mb-3 pb-2 border-bottom">
                        <h6 class="fw-bold mb-0">
                            <i class="bi bi-layers text-warning me-2"></i>{{ __('cms.products.variants') }}
                        ...
                        <div class="variant-block bg-light rounded-3 p-3 mb-3" data-index="{{ $index }}">
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                <span class="badge bg-primary">
                                    <i class="bi bi-tag-fill me-1"></i>{{ __('cms.products.variant_label', ['num' => $index + 1]) }}
                                </span>
                                <button type="button" class="btn btn-sm btn-outline-danger remove-variant-item"
                                    onclick="removeVariant({{ $variant->id ?? 'null' }}, this)">
                                    <i class="bi bi-trash3 me-1"></i>{{ __('cms.products.remove') }}
                                </button>
                            </div>

                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold small">{{ __('cms.products.variant_name_en') }}</label>
                                ...
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold small">{{ __('cms.products.price') }}</label>
                                ...
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold small">{{ __('cms.products.discount_price') }}</label>
                                    <div class="input-group">
                                        <span class="input-group-text bg-white border-0 text-muted">$</span>
                                ...
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold small">{{ __('cms.products.stock') }}</label>
                                    <input type="number" name="variants[{{ $index }}][stock]"
                                        class="form-control bg-white border-0"
                                        value="{{ old("variants.{$index}.stock", $variant->stock) }}" />
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold small">{{ __('cms.products.sku') }}</label>
                                    <input type="text" name="variants[{{ $index }}][SKU]"
                                        class="form-control bg-white border-0"
                                        value="{{ old("variants.{$index}.SKU", $variant->SKU) }}" />
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold small">{{ __('cms.products.barcode') }}</label>
                                    <input type="text" name="variants[{{ $index }}][barcode]"
                                        class="form-control bg-white border-0"
                                        value="{{ old("variants.{$index}.barcode", $variant->barcode) }}" />
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold small">{{ __('cms.products.weight') }}</label>
                                    <input type="text" name="variants[{{ $index }}][weight]"
                                        class="form-control bg-white border-0"
                                        value="{{ old("variants.{$index}.weight", $variant->weight) }}" />
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold small">{{ __('cms.products.dimension') }}</label>
                                    <input type="text" name="variants[{{ $index }}][dimensions]"
                                        class="form-control bg-white border-0"
                                        value="{{ old("variants.{$index}.dimensions", $variant->dimensions) }}" />
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold small">{{ __('cms.products.size') }}</label>
                                    <select name="variants[{{ $index }}][size_id]" class="form-select bg-white border-0">
                                        <option value="">{{ __('cms.products.no_size') }}</option>
                                        @foreach($sizes as $size)
                                        <option value="{{ $size->id }}" {{ old("variants.{$index}.size_id", $variant->size_id) == $size->id ? 'selected' : '' }}>{{ $size->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold small">{{ __('cms.products.color') }}</label>
                                    <select name="variants[{{ $index }}][color_id]" class="form-select bg-white border-0">
                                        <option value="">{{ __('cms.products.no_color') }}</option>
                                        @foreach($colors as $color)
                                        <option value="{{ $color->id }}" {{ old("variants.{$index}.color_id", $variant->color_id) == $color->id ? 'selected' : '' }}>{{ $color->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                    ...
                    <div id="no-variants-msg" class="text-center py-5" style="{{ count($product->variants) > 0 ? 'display:none' : '' }}">
                        ...
                        <p class="text-muted small mt-2 mb-0">{{ __('cms.products.no_variants_msg', ['button' => '<strong>' . __('cms.products.add_variant') . '</strong>']) }}</p>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-body p-4">
                    <h6 class="fw-bold mb-3 pb-2 border-bottom">
                        <i class="bi bi-diagram-3 text-success me-2"></i>{{ __('cms.products.section_organization') }}
                    </h6>
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-body p-4">
                    <h6 class="fw-bold mb-3 pb-2 border-bottom">
                        <i class="bi bi-images text-info me-2"></i>{{ __('cms.products.images') }}
                    </h6>
                    ...
                    <label class="btn btn-outline-light border-dashed text-primary w-100 py-3 d-flex flex-column align-items-center gap-1"
                        ...
                        <span class="fw-semibold small">{{ __('cms.products.upload_images_cta') }}</span>
                        <span class="text-muted" style="font-size:.72rem;">{{ __('cms.products.upload_images_hint') }}</span>
...
<template id="variant-template">
    <div class="variant-block bg-light rounded-3 p-3 mb-3" data-index="__INDEX__">
        <div class="d-flex align-items-center justify-content-between mb-3">
            <span class="badge bg-primary">
                <i class="bi bi-tag-fill me-1"></i>{{ __('cms.products.variant_label', ['num' => '__NUM__']) }}
            </span>
            <button type="button" class="btn btn-sm btn-outline-danger remove-variant-item">
                <i class="bi bi-trash3 me-1"></i>{{ __('cms.products.remove') }}
            </button>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-3 d-flex align-items-end">
                <div class="form-check form-switch mb-1">
                    ...
                    <label class="form-check-label small fw-semibold" for="primary___INDEX__">{{ __('cms.products.is_primary') }}</label>
                </div>
            </div>
        </div>
    </div>
